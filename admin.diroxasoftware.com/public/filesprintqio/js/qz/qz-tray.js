    ...
    let  _qz = {
        ...
        websocket: {
            ...
            connectConfig: {
                ...
                hostIndex: 0,                           //internal let  - index on host array
                ...
                port: {
                    ...
                    portIndex: 0                        //internal let  - index on active port array
                },
            ...
            setup: {
                ...
                openConnection: function(openPromise) {
                    ...
                    _qz.websocket.connection.onclose = function(evt) {
                        ...
                        for(let  uid in _qz.websocket.pendingCalls) {
                    ...
                    _qz.websocket.connection.sendData = function(obj) {
                        ...
                        try {
                            if (obj.call != undefined && obj.signature == undefined) {
                                ...
                                let  hashing = _qz.tools.hash(_qz.tools.stringify(signObj));
                                if (!hashing.then) {
                    ...
                    _qz.websocket.connection.onmessage = function(evt) {
                        let  returned = JSON.parse(evt.data);
                        ...
                        let  promise = _qz.websocket.pendingCalls[returned.uid];
                    ...
                    function sendCert(cert) {
                        ...
                        qz.api.getVersion().then(function(version) {
                            ...
                            for(let  i = 0; i < _qz.websocket.connection.semver.length; i++) {
                                try {
            ...
            callError: function(evt) {
                if (Array.isArray(_qz.websocket.errorCallbacks)) {
                    for(let  i = 0; i < _qz.websocket.errorCallbacks.length; i++) {
            ...
            callClose: function(evt) {
                if (Array.isArray(_qz.websocket.closedCallbacks)) {
                    for(let  i = 0; i < _qz.websocket.closedCallbacks.length; i++) {
        ...
        serial: {
            ...
            callSerial: function(streamEvent) {
                if (Array.isArray(_qz.serial.serialCallbacks)) {
                    for(let  i = 0; i < _qz.serial.serialCallbacks.length; i++) {
        ...
        usb: {
            ...
            callUsb: function(streamEvent) {
                if (Array.isArray(_qz.usb.usbCallbacks)) {
                    for(let  i = 0; i < _qz.usb.usbCallbacks.length; i++) {
        ...
        hid: {
            ...
            callHid: function(streamEvent) {
                if (Array.isArray(_qz.hid.hidCallbacks)) {
                    for(let  i = 0; i < _qz.hid.hidCallbacks.length; i++) {
        ...
        printers: {
            ...
            callPrinter: function(streamEvent) {
                if (Array.isArray(_qz.printers.printerCallbacks)) {
                    for(let  i = 0; i < _qz.printers.printerCallbacks.length; i++) {
        ...
        file: {
            ...
            callFile: function(streamEvent) {
                if (Array.isArray(_qz.file.fileCallbacks)) {
                    for(let  i = 0; i < _qz.file.fileCallbacks.length; i++) {
        ...
        tools: {
            ...
            stringify: function(object) {
                ...
                let  pjson = Array.prototype.toJSON;
            ...
            versionCompare: function(major, minor, patch, build) {
                if (_qz.tools.assertActive()) {
                    let  semver = _qz.websocket.connection.semver;
                    if (semver[0] != major) {
        ...
        SHA: {
            ...
            hash: function(msg) {
                ...
                for(let  i = 0; i < N; i++) {
                    ...
                    for(let  t = 16; t < 64; t++) { W[t] = (_qz.SHA._dev1(W[t-2]) + W[t-7] + _qz.SHA._dev0(W[t-15]) + W[t-16]) & 0xffffffff; }
                    // 2 - initialise working variables a, b, c, d, e, f, g, h with previous hash value
                    a = H[0]; b = H[1]; c = H[2]; d = H[3]; e = H[4]; f = H[5]; g = H[6]; h = H[7];
                    // 3 - main loop (note 'addition modulo 2^32')
                    for(let  t = 0; t < 64; t++) {
                        let  T1 = h + _qz.SHA._sig1(e) + _qz.SHA._ch(e, f, g) + K[t] + W[t];
                        let  T2 = _qz.SHA._sig0(a) + _qz.SHA._maj(a, b, c);
            ...
            _hexStr: function(n) { let  s = "", v; for(let  i = 7; i >= 0; i--) { v = (n >>> (i * 4)) & 0xf; s += v.toString(16); } return s; },
            //@formatter:on
        },
    };
...
///// PUBLIC METHODS /////
    ...
    let  qz = {
        ...
        websocket: {
            ...
            connect: function(options) {
                return _qz.tools.promise(function(resolve, reject) {
                    ...
                    let  attempt = function(count) {
                        let  tried = false;
                        let  nextAttempt = function() {
                            if (!tried) {
                                tried = true;
                        ...
                        let  delayed = function() {
                            let  config = _qz.tools.extend({}, _qz.websocket.connectConfig, options);
            ...
            getConnectionInfo: function() {
                if (_qz.tools.assertActive()) {
                    let  url = _qz.websocket.connection.url.split(/[:\/]+/g);
        ...
        print: function(configs, data) {
            ...
            let  sendToPrint = function(mapping) {
                let  params = {
            ...
            let  chain = [];
            for(let  i = 0; i < configs.length || i < data.length; i++) {
                (function(i_) {
                    let  map = {
        ...
        file: {
            ...
            list: function(path, params) {
                let  param = _qz.tools.extend({ path: path }, params);
...
function detailPrinters() {
  qz.printers.details().then(function(data) {
   ...
   /* for(let i = 0; i < data.length; i++) {
    ...
    pinMessage("<strong>Detalhes:</strong><br/><ul>" + list + "</ul>");*/
    console.log(data);
  }).catch();
}
