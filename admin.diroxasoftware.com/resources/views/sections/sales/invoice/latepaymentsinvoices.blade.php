...
<div class="container-fluid invoice-container">
  ...
  <main>
  <div class="row">
    <div class="col-sm-6" style="color:black;"><strong>Date:</strong> {{$created_at}}</div>
  ...
  <div class="row p-2">
      ...
      @foreach($ProductsInfos2 as $product)
             <?php
                    ...
                    $totalSalesWithoutVat =  $product->totalSalesWithoutVat;

                    $totalPaidSoFar = $salesInfos->sales_price;

                    // $upfrontValue = number_format($salesInfos->upfrontValue, 2, '.',',');
                    $payToday = number_format($salesInfos->payToday, 2, '.',',');
                    $upfrontValue = number_format($salesInfos->upfrontValue, 2, '.',',');
                ...
                @else
                    <div class="alert alert-info">
                      @if($paymentsOptions == "PAYING WEEKLY")
                        ...
                        <div class="col-md-7">
                          Last Bill was  charged on: {{$salesInfos->lastPaymentDate}}
                        </div>
                        </div>
                    </div>
                @endif
...
@elseif($typesales == 1)
    <div class="row p-3">
            ...
            <!-- <strong><p class="font-weight-bold mb-4" style="color:black;">Products Selected</p></strong> -->
                ...
                <p class="font-weight-bold mb-4" style="color:black;">Payment method: <strong>{{$salesInfos->PaymentOption}}</strong></p>
            </div>
        </div>
        @endif
