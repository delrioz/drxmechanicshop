...
<div class="container-fluid invoice-container">
  ...
  <div class="row">
    ...
    <div class="col-sm-6 text-sm-right"> <strong>Invoice No:</strong> #{{$ThisWkInvoice}}</div>
  ...
  <div class="row">
    ...
    <div class="col-sm-6 order-sm-0"> <strong>Invoiced To:</strong>
      <address>
      <strong>{{$findDatasCustomer->name}}<br /></strong>
      {{$findDatasCustomer->email}}<br />
      {{$findDatasCustomer->telephone}}<br />
      {{$findDatasCustomer->address}}<br />
      United Kingdom
      </address>
    </div>
  </div> 
  ...
  <div class="row p-3">
      ...
      @foreach($ProductsInfo2 as $prod)
                <tbody>
                    <tr>
                        ...
                        <td>{{$prod->productQuantityPmw}}</td>
                        <td>£{{$prod->productSellPrice}}</td>
                        <td>£{{$prod->productQuantityPmw * $prod->productSellPrice }}</td>
                    </tr>
                </tbody>
        @endforeach
            </table>
            <div class="row">
            <div class="col-md-6">
              <p class="font-weight-bold">Payment method: <strong>{{$workorder_payments->typeofpayment}}</strong></p>
            </div>
    ...
    <div class="card-body px-2">
        ...
        <table class="table">
          <tbody>
            <tr>
              ...
              <td class="bg-light-2 text-right">£{{$workorder_payments->amount}}</td>
            </tr>
            <tr>
              <td colspan="4" class="bg-light-2 text-right"><strong>Vat:</strong></td>
              <td class="bg-light-2 text-right">£{{$workorder_payments->vat}}</td>
            </tr>
            <tr>
            <tr>
              <td colspan="4" class="bg-light-2 text-right"><strong>Discount:</strong></td>
              <td class="bg-light-2 text-right">£{{$workorder_payments->discount}}</td>
            </tr>
            <tr>
            <tr>
              <td colspan="4" class="bg-light-2 text-right"><strong>Work Labor:</strong></td>
              <td class="bg-light-2 text-right">£{{$workorder_payments->worklabor}}</td>
            </tr>
            <tr>
              <td colspan="4" class="bg-light-2 text-right"><strong>Total:</strong></td>
              <td class="bg-light-2 text-right">£{{$workorder_payments->totalWithVAT}}</td>
          </tbody>
        </table>
      </div>
      ...
      <a href="/section/machines/viewPage/{{$Machine_Id}}" class="btn btn-warning border text-black-50 shadow-none"><i class="fa fa-back"></i> Back</a> </div>
